...
static double fc (double t, double y)
{
  N++;
  return y - t*t + 1.0;
}

static double Sfc (double t)
{
  return (t + 1.0)*(t + 1.0) - (1.0/2.0) * exp(t);
}

static double fd (double t, double y)
{
  N++;
  return cos(t);
}

static double Sfd (double t)
{
  return sin(t);
}
...
int main (void)
{
  ...
  printf("--- Teste de Runge-Kutta (c) ---\n");
  teste_runge_kutta(0, 1.0, 0.001, 0.5, fc, Sfc);
  printf("--- Teste de Runge-Kutta (d) ---\n");
  teste_runge_kutta(0, 2*PI, 0.01, 0.0, fd, Sfd); 
  return 0;
}
