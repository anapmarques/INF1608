...
void testmsa (double a, double b, double (*f) (double), double tol)
{
  feval = 0;
  double xmin;
  int n = msa(a, b, f, TOL_MSA, &xmin); // Completar com chamada do método da seção áurea
...
void testmips (double r, double delta, double (*f) (double), double tol)
{
  feval = 0;
  double xmin;
  int n = mips(r, delta, f, TOL_MIPS, &xmin); // Completar com chamada do método da interpolação parabolica sucessiva
